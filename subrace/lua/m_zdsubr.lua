zZsubrTitleTable = {
	...
	['%Ztext_changeling%'] = {25, %changeling_desc%},
}
...
function subt(chr)
	...
	elseif (chr == 24) then return Infinity_FetchString(%changeling_title%) --subrStrings.CHANGELING
	else return ""
	end
end
...
function createSubraceTable(race)
	...
	if race == 1 then --human
		sbSubraceData = {
			...
			{%Zchangeling_title%, %changeling_desc%, 25},
		}
	end
	if race == 2 then --elf
		sbSubraceData = {
